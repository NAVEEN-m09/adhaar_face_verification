...
def get_records(db: Session = Depends(get_db), current_user: AdminUser = Depends(get_current_admin)):
    ...
    for r in records:
        result.append({
            ...
            "face_matched": r.selfie_similarity >= 59.0 if r.selfie_similarity else False,
            "third_doc_matched": r.third_doc_similarity >= 59.0 if r.third_doc_similarity else False,
            "status": r.status,
